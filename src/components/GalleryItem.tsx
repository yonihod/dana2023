...
export default function GalleryItem({ item, setShow, show }: { item: Item, setShow: any, show: boolean }) {
    ...
    if(!item) return null;
    return (
        <div className={`fixed top-0 left-0 h-full w-full bg-black bg-opacity-80 text-white transition-opacity duration-300 ease-in-out ${show ? 'z-10 opacity-100' : 'opacity-0 -z-10'}`}>
            <div className="relative w-full h-full">
                <button className="absolute top-2 right-2 text-2xl z-40" onClick={() => setShow(false)}>
                    x
                </button>
